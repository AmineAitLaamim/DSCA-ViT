# ============================================================
# UNI-RGB-MLP — Training Script (CLI, HPC-ready)
# ============================================================
#
...
#
# Usage:
#   uv run python UNI-RGB-MLP/train_uni_rgb_mlp.py \
...
def load_split_indices(split_indices_path: str) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    if not path.exists():
        raise FileNotFoundError(
            ...
            "it is never regenerated by the UNI-RGB-MLP."
        )
    data = np.load(path)
...
def setup_logging(log_dir: str, rank: int = 0) -> logging.Logger:
    logger = logging.getLogger("uni_rgb_mlp")
...
def build_optimizer(model: nn.Module, cfg: dict) -> torch.optim.Optimizer:
    """AdamW on head only, weight decay on weights only."""
        ...
        for token in forbidden:
            if token in text:
                raise RuntimeError(
                    ...
                    "The UNI-RGB-MLP must be fully offline."
                )
...
def run_sanity_check(model, device, num_classes=4, bs=2, image_size=224, device_name="cpu") -> None:
    print("=" * 60)
    print("SANITY CHECK — UNI-RGB-MLP")
    print("=" * 60)
...
def run_training(config: dict, resume: bool, debug: bool, force_distributed: bool = False) -> None:
    ...
    if rank == 0:
        counts = model.count_parameters()
        logger.info("=" * 60)
        logger.info("UNI-RGB-MLP Parameter Summary")
        logger.info("=" * 60)
        for name, count in counts.items():
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train UNI-RGB-MLP (HER2-IHC-40x)")
