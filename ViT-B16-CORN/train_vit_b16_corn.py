# ============================================================
# ViT-B16-CORN — Training Script (CLI, HPC/DDP-ready)
# ============================================================
#
# Reproduces the proper ViT-B16 baseline protocol (2-stage),
# with the ONLY change being the output layer and loss: standard
# 4-way cross-entropy is replaced by CORN ordinal regression
# (coral-pytorch).
#
# Corlliery:
#   Stage 1 (30 ep): frozen ViT backbone, train CORN head only, lr=1e-4
#   Stage 2 (30 ep): full fine-tuning, backbone lr=1e-5, head lr=1e-4
#
#   - Optimizer : Adam
#   - Weight decay: 0.0
...
#
# Usage:
#   uv run python ViT-B16-CORN/train_vit_b16_corn.py \
#       --config configs/vit_b16_corn_config.yaml
#   ... --resume
#   ... --debug
# ============================================================
...
# ------------------------------------------------------------
# Transforms — ImageNet normalization IN THE DATALOADER
# (matches the proper ViT-B16 baseline convention).
# ------------------------------------------------------------
...
def load_split_indices(split_indices_path: str) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    if not path.exists():
        raise FileNotFoundError(
            ...
            "it is never regenerated by the ViT-B16-CORN."
        )
    data = np.load(path)
...
def setup_logging(log_dir: str, rank: int = 0) -> logging.Logger:
    logger = logging.getLogger("vit_b16_corn")
        ...
        for token in forbidden:
            if token in text:
                raise RuntimeError(
                    ...
                    "The ViT-B16-CORN must be fully offline and must not "
...
def run_sanity_check(
    ...
    print("=" * 60)
    print("SANITY CHECK — ViT-B16-CORN")
    print("=" * 60)
    ...
    # Verify the transform pipeline has ImageNet Normalize.
    train_transform = get_train_transform(image_size=image_size)
    has_normalize = False
    for tr in train_transform.transforms:
        if isinstance(tr, transforms.Normalize):
            if list(tr.mean) == IMAGENET_MEAN and list(tr.std) == IMAGENET_STD:
                has_normalize = True
    if not has_normalize:
        raise RuntimeError("Train transform pipeline missing ImageNet Normalize!")
    print("  Dataloader Normalize(ImageNet) : OK")

    model.eval()
    x = torch.rand(bs, 3, image_size, image_size, device=device)  # normalized-ish random
    with torch.no_grad():
        pred = model(x)
...
def run_training(config: dict, resume: bool, debug: bool, force_distributed: bool = False) -> None:
    ...
    if rank == 0:
        counts = model.count_parameters()
        logger.info("=" * 60)
        logger.info("ViT-B16-CORN Parameter Summary")
        logger.info("=" * 60)
        for name, count in counts.items():
    ...
    # AMP disabled (matches proper baseline).
    use_amp = False
    scaler = None
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train ViT-B16-CORN (HER2-IHC-40x)")
    parser.add_argument("--config", type=str, default="configs/vit_b16_corn_config.yaml")
