# ============================================================
# UNI-Stain-EarlyFusion — Training Script (CLI, HPC-ready)
# ============================================================
#
...
def setup_logging(log_dir: str, rank: int = 0) -> logging.Logger:
    logger = logging.getLogger("uni_stain_earlyfusion")
...
def set_stage_requires_grad(model: nn.Module, stage: int) -> None:
    ...
    if stage == 1:
        # UNI backbone frozen, head trainable.
        ...
        for token in forbidden:
            if token in text:
                raise RuntimeError(
                    ...
                    "The UNI-Stain-EarlyFusion must be fully offline."
                )
...
def run_sanity_check(model, device, num_classes=4, bs=2, image_size=224, device_name="cpu") -> None:
    print("=" * 60)
    print("SANITY CHECK — UNI-Stain-EarlyFusion")
    print("=" * 60)
    ...
    # Patch-embed projection is 5-channel (early fusion)
    proj = model.backbone.patch_embed.proj
    if proj.in_channels != 5:
        raise RuntimeError(
            f"patch_embed.proj.in_channels must be 5, got {proj.in_channels}."
        )
    print(f"  Patch-embed in_channels: {proj.in_channels} (OK)")

    # RGB-slice copied from original UNI proj; H/DAB zero-init
    if not torch.equal(proj.weight[:, :3], model._orig_proj_weight):
        raise RuntimeError("RGB projection channels do NOT match original UNI.")
    print("  RGB proj channels match original UNI : OK")
    if not torch.all(proj.weight[:, 3:] == 0):
        raise RuntimeError("H/DAB projection channels are NOT zero-init.")
    print("  H/DAB proj channels zero-initialized   : OK")

    # Confirm UNI checkpoint was loaded before replace
    if not getattr(model, "_uni_loaded", False):
        raise RuntimeError("UNI checkpoint not loaded before projection replace.")
    print("  UNI checkpoint loaded before replace   : OK")
...
def run_training(config: dict, resume: bool, debug: bool, force_distributed: bool = False) -> None:
    ...
    if not os.path.exists(stain_stats_path):
        raise FileNotFoundError(
            ...
            "Run UNI-Stain-EarlyFusion/precompute_stain_stats.py first."
        )
    stain_stats = load_stain_stats(stain_stats_path)

    # Model — strict-load-before-replace of the 5-channel projection.
    model = UNIStainEarlyFusion(
    ...
    if rank == 0:
        counts = model.count_parameters()
        logger.info("=" * 60)
        logger.info("UNI-Stain-EarlyFusion Parameter Summary")
        logger.info("=" * 60)
        for name, count in counts.items():
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train UNI-Stain-EarlyFusion (HER2-IHC-40x)")
    parser.add_argument("--config", type=str, default="configs/uni_stain_earlyfusion_config.yaml")
