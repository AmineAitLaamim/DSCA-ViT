# ============================================================
# UNI-Stain-MLP — Official Test Evaluation Script (CLI)
# ============================================================
#
# Loads the best checkpoint (best.pt — NEVER last.pt) and
# evaluates on the official test set exactly once.
#
# Reports accuracy, balanced accuracy, macro-F1, weighted-F1,
# per-class P/R/F1, QWK, and confusion matrix.
#
# Usage:
#   uv run python UNI_v2/evaluate_uni_stain_mlp.py \
#       --config configs/uni_stain_mlp_config.yaml
# ============================================================
...
try:
    ...
    # Self-contained fallback (keeps UNI_v2/ usable standalone).
...
# Sibling imports (UNI_v2/ package, run by path).
from uni_stain_mlp import UNIStainMLP
from stain_stats import load_stain_stats
from train_uni_stain_mlp import get_test_transform
...
def setup_logging(log_dir: str) -> logging.Logger:
    logger = logging.getLogger("uni_stain_mlp_eval")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate UNI-Stain-MLP on Official Test Set")
    parser.add_argument("--config", type=str, default="configs/uni_stain_mlp_config.yaml")
...
def main() -> None:
    ...
    checkpoint_path = args.checkpoint or os.path.join(checkpoint_dir, "best.pt")
    if not os.path.exists(checkpoint_path):
        raise FileNotFoundError(
            f"Checkpoint not found at '{checkpoint_path}'. "
            "Train the model first (best.pt is required) or provide --checkpoint."
        )
    ...
    model.eval()
    logger.info("Model loaded from checkpoint (best.pt).")

    # Official test set — transforms have NO ImageNet normalization.
    ...
    print("=" * 60)
    print("UNI-Stain-MLP — Official Test Evaluation")
    print("=" * 60)
    ...
    with open(report_path, "w") as f:
        f.write("=" * 60 + "\n")
        f.write("UNI-Stain-MLP — Official Test Evaluation\n")
        f.write("=" * 60 + "\n\n")
