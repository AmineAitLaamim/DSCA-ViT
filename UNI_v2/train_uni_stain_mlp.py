# ============================================================
# UNI-Stain-MLP — Training Script (CLI, HPC-ready)
# ============================================================
#
# Single-stage training. UNI backbone is frozen FOREVER.
# Only the StainEncoder branch and MLP head are trainable.
#
#   Epochs        : 30
#   Optimizer     : AdamW (stain_encoder + head params only)
#   LR            : 1e-3
...
#
# Usage:
#   uv run python UNI_v2/train_uni_stain_mlp.py \
...
def load_split_indices(split_indices_path: str) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    if not path.exists():
        raise FileNotFoundError(
            ...
            "it is never regenerated by the UNI-Stain-MLP."
        )
    data = np.load(path)
...
def setup_logging(log_dir: str, rank: int = 0) -> logging.Logger:
    logger = logging.getLogger("uni_stain_mlp")
...
# ------------------------------------------------------------
# Optimizer — only stain_encoder + head parameters
# ------------------------------------------------------------
def build_optimizer(model: nn.Module, cfg: dict) -> torch.optim.Optimizer:
    """AdamW on stain_encoder + head only, weight decay on weights only."""
    wd = cfg["training"]["weight_decay"]

    trainable_params = list(model.stain_encoder.parameters()) + list(model.head.parameters())
        ...
        for token in forbidden:
            if token in text:
                raise RuntimeError(
                    ...
                    "The UNI-Stain-MLP must be fully offline."
                )
...
def run_sanity_check(model, device, num_classes=4, bs=2, image_size=224, device_name="cpu") -> None:
    print("=" * 60)
    print("SANITY CHECK — UNI-Stain-MLP")
    print("=" * 60)
...
def run_training(config: dict, resume: bool, debug: bool, force_distributed: bool = False) -> None:
    ...
    if not os.path.exists(stain_stats_path):
        raise FileNotFoundError(
            ...
            "Run UNI_v2/precompute_stain_stats.py with the same config first."
        )
    ...
    if rank == 0:
        counts = model.count_parameters()
        logger.info("=" * 60)
        logger.info("UNI-Stain-MLP Parameter Summary")
        logger.info("=" * 60)
        for name, count in counts.items():
            logger.info(f"  {name:<16} : {count:>12,}")
        logger.info("=" * 60)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train UNI-Stain-MLP (HER2-IHC-40x)")
