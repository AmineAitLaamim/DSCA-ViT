# ============================================================
# DSS-ViT v2.2 — Main Architecture Assembly
# ============================================================
#
# Dual-Stream Stain Vision Transformer for HER2 IHC Scoring.
#
# v2.2 changes (vs v2.1):
#   - StainEncoder: num_tokens 16 -> 8, bottleneck 512 -> 256
#   - Same cross-attention + gate + ordinal head
...
class DSSViT(nn.Module):
    """
    Dual-Stream Stain Vision Transformer (v2.2).
    ...
    Args:
        ...
        num_stain_tokens (int): Number of stain tokens (default 8 for v2.2).
        stain_bottleneck_dim (int): StainEncoder bottleneck dim (default 256 for v2.2).
    ...
    def __init__(
        ...
        num_stain_tokens: int = 8,
        stain_bottleneck_dim: int = 256,
        ...
        # --------------------------------------------------------
        # 4. StainEncoder (auxiliary branch, reduced capacity)
        # --------------------------------------------------------
    ...
    def forward(self, x_rgb: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        # --------------------------------------------------------
        stain_tokens = self.stain_encoder(stain_input)  # (B, 8, 768)
        ...
        # Key/Value: stain_tokens (B, 8, 768)
