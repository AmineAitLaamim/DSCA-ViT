# ============================================================
# DSS-ViT v2.2 — Color Deconvolution (self-contained copy)
# ============================================================
#
# This is a self-contained copy of the fixed Ruifrok & Johnston
# ColorDeconvolution from models_v2_1/color_deconv.py. It is
# physics-based (non-learnable) and identical to the original.
#
# The original models/, models_v2/, models_v3/, models_v2_1/
# are NOT modified.
# ============================================================
...
class ColorDeconvolution(nn.Module):
    ...
    def __init__(self, epsilon: float = 1e-6) -> None:
        """
        Initializes the ColorDeconvolution module with the fixed H-DAB
        stain matrix.

        Args:
            epsilon (float, optional): A small value to avoid log(0) when
                                       converting to Optical Density space.
                                       Defaults to 1e-6.
        """
        super().__init__()
        ...
        # Register as a buffer so it is automatically moved to the correct
        # device (CPU/GPU) but not updated during backpropagation.
...
def deconvolve_numpy(image_rgb_uint8: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    ...
    Returns:
        Tuple[np.ndarray, np.ndarray]: A tuple containing the Hematoxylin and
                                       DAB channels as float32 numpy arrays
                                       of shape (H, W).
    """
