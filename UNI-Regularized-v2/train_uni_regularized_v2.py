# ============================================================
# UNI-Regularized-v2 — Training Script (CLI, HPC/DDP-ready)
# ============================================================
#
...
#
# v2 bundles THREE regularization changes (kept together in
# this single run):
#
#   1. Stochastic depth  : UNI backbone created with
#                          drop_path_rate=0.2 (configurable).
#                          This adds DropPath modules WITHOUT
#                          changing parameter count, so the
#                          strict load_state_dict from the
#                          official UNI weights is unaffected.
#
#   2. Head dropout      : the linear head is wrapped as
#                          nn.Sequential(nn.Dropout(0.3),
#                          nn.Linear(1024, num_classes)).
#                          No extra hidden layers.
#
#   3. Rebalanced wd      : Stage 2 backbone weight decay is
#                          raised to 0.5 so that lr*wd parity
#                          matches the head:
#                          backbone: 1e-5 * 0.5 = 5.00e-06
#                          head     : 1e-4 * 0.05 = 5.00e-06
#
# Everything else is identical to v1 (2-stage recipe, split
# file, transforms, early stopping, checkpoint selection).
#
...
#       AdamW backbone 1e-5 (wd 0.5 non-1D / 0.0 1D),
...
#   This experiment is evaluated by comparing TRAINING CURVES
#   against v1. The per-epoch console line therefore includes
#   Train Acc (v1's console output did NOT include Train Acc):
#
...
#
#   After each stage, the stop reason is logged:
...
def setup_logging(log_dir: str, rank: int = 0) -> logging.Logger:
    logger = logging.getLogger("uni_regularized_v2")
...
def set_stage_requires_grad(model: nn.Module, stage: int) -> None:
    ...
    The head must remain trainable in BOTH stages (symmetry fix —
    the --debug sanity check asserts head requires_grad after both
    set_stage_requires_grad(model, 1) and (model, 2)).
    """
    if stage == 1:
...
def build_optimizer(model: nn.Module, cfg: dict, stage: int) -> torch.optim.Optimizer:
    ...
    if stage == 2:
        # v2: backbone and head use DIFFERENT weight decay so that
        # lr*wd is equalized: 1e-5*0.5 = 1e-4*0.05 = 5.00e-06.
        wd_s2_backbone = cfg_training.get("weight_decay_stage2_backbone", 0.5)
        wd_s2_head = cfg_training.get("weight_decay_stage2_head", 0.05)
        backbone_decay, backbone_no_decay = split_decay_params(model.backbone)
        head_decay, head_no_decay = split_decay_params(model.head)
        groups = []
        if backbone_decay:
            groups.append({"params": backbone_decay, "lr": cfg_training["stage2_backbone_lr"], "weight_decay": wd_s2_backbone})
        if backbone_no_decay:
            groups.append({"params": backbone_no_decay, "lr": cfg_training["stage2_backbone_lr"], "weight_decay": 0.0})
        if head_decay:
            groups.append({"params": head_decay, "lr": cfg_training["stage2_head_lr"], "weight_decay": wd_s2_head})
        if head_no_decay:
...
def run_sanity_check(
    ...
    print("=" * 60)
    print("SANITY CHECK — UNI-Regularized-v2")
    print("=" * 60)
    ...
    model.train()  # DropPath + head dropout active for backward
    pred2 = model(x2)
    ...
    # DropPath active check (stochastic depth) — class-name based,
    # no hardcoded timm import path.
    drop_path_modules = [
        m for m in model.backbone.modules()
        if type(m).__name__ == "DropPath" and getattr(m, "drop_prob", 0) > 0
    ]
    assert len(drop_path_modules) > 0, \
        "drop_path_rate did not produce active DropPath modules"
    print(f"  DropPath active    : OK ({len(drop_path_modules)} modules, "
          f"e.g. drop_prob={getattr(drop_path_modules[0], 'drop_prob', None)})")

    # Head dropout check.
    assert any(isinstance(m, nn.Dropout) for m in model.head.modules()), \
        "head dropout missing"
    print("  Head dropout       : OK (nn.Dropout present in head)")

    # Stage 1: backbone frozen, head trainable.
    set_stage_requires_grad(model, 1)
    assert all(not p.requires_grad for p in model.backbone.parameters()), \
        "Stage 1: some backbone params are still trainable!"
    assert all(p.requires_grad for p in model.head.parameters()), \
        "Stage 1: head params should be trainable!"
    print("  Stage 1 requires_grad: OK (backbone frozen, head trainable)")

    # Stage 2: ALL params trainable — head must STILL be trainable
    # (symmetry fix).
    set_stage_requires_grad(model, 2)
    assert all(p.requires_grad for p in model.backbone.parameters()), \
        "Stage 2: some backbone params are still frozen!"
    assert all(p.requires_grad for p in model.head.parameters()), \
        "Stage 2: head params should remain trainable!"
    print("  Stage 2 requires_grad: OK (all backbone + head trainable)")
...
def run_training(config: dict, resume: bool, debug: bool, force_distributed: bool = False) -> None:
    ...
    # Model.
    model = UNIRegularizedV2(
        checkpoint_path=uni_checkpoint_path,
        num_classes=config["model"]["num_classes"],
        drop_path_rate=config["model"].get("drop_path_rate", 0.2),
    ...
    # Effective decay strength logging (v2 requirement) — log lr*wd
    # parity so it is auditable directly from the training log.
    backbone_lr = config["training"].get("stage2_backbone_lr", 1.0e-5)
    backbone_wd = config["training"].get("weight_decay_stage2_backbone", 0.5)
    head_lr = config["training"].get("stage2_head_lr", 1.0e-4)
    head_wd = config["training"].get("weight_decay_stage2_head", 0.05)
    logger.info("=" * 60)
    logger.info(f"Early stopping: patience={patience}, min_delta={min_delta} (on val loss)")
    logger.info(f"Effective backbone decay strength (lr*wd): {backbone_lr * backbone_wd:.2e}")
    logger.info(f"Effective head decay strength (lr*wd): {head_lr * head_wd:.2e}")
    logger.info("=" * 60)
    ...
    for epoch in range(start_epoch_s1, stage1_epochs):
        ...
        if rank == 0:
            ...
            # v2 REQUIRED LOG FORMAT WITH Train Acc (exact):
            logger.info(
...
def _debug_stage2_init_check(model, device, checkpoint_dir, config) -> None:
    ...
    # Verify restoration by comparing ALL head tensors.
    # (v2 head is nn.Sequential — use state_dict keys, not getattr.)
    head_after = state_model.head.state_dict()
    for name in head_before:
        assert torch.allclose(
            head_after[name], head_before[name]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train UNI-Regularized-v2 (HER2-IHC-40x)")
    parser.add_argument("--config", type=str, default="configs/uni_regularized_v2_config.yaml")
