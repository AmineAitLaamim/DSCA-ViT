...
#
# There are NO network calls and NO Hugging Face downloads
# anywhere in this file; weights are loaded strictly from the
# local checkpoint file.
#
# The dataloader returns RAW RGB in [0,1] — ImageNet
# normalization is applied INSIDE forward() so the data
# pipeline stays identical to the plain ViT baseline.
# ============================================================
...
# Must be set BEFORE any timm/torch/huggingface imports.
...
class UNIBaselineModel(nn.Module):
    ...
    def __init__(self, checkpoint_path: str, num_classes: int = 4, verbose: bool = False):
        ...
        if verbose:
            print(f"  [UNI] Loaded backbone strictly from '{checkpoint_path}'")
