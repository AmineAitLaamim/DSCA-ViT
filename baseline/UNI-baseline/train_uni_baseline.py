# ============================================================
# UNI Baseline — Training Script (CLI, HPC-ready)
# ============================================================
#
# Trains the UNI histopathology foundation model (ViT-L/16)
# with a 4-class head on HER2-IHC-40x using the SAME proper
# protocol as the plain ViT-B16 baseline:
#
#   Stage 1 (30 epochs): frozen UNI backbone, train head only, lr=1e-4
...
# Import the UNI model via direct sibling import (hyphen folder, run by path).
...
def setup_logging(log_dir: str, rank: int = 0) -> logging.Logger:
    logger = logging.getLogger("uni_baseline")
        ...
        for token in forbidden:
            if token in text:
                raise RuntimeError(
                    ...
                    "The UNI baseline must be fully offline."
                )
...
def run_sanity_check(model, device, num_classes=4, bs=2, image_size=224, device_name="cpu") -> None:
    print("=" * 60)
    print("SANITY CHECK — UNI Baseline")
    print("=" * 60)
...
def run_training(config: dict, resume: bool, debug: bool, force_distributed: bool = False) -> None:
    ...
    if rank == 0:
        counts = model.count_parameters()
        logger.info("=" * 60)
        logger.info("UNI Baseline Parameter Summary")
        logger.info("=" * 60)
        for name, count in counts.items():
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train UNI Baseline (HER2-IHC-40x)")
    parser.add_argument("--config", type=str, default="configs/uni_baseline_config.yaml")
